                    ...
                    @foreach ($cinemas as $cinema)
                        <div class="cinema-row mb-3 p-3" style="border: 1px solid #fff; border-radius: 5px;">
                            <div class="form-check form-switch">
                                <input class="form-check-input cinema-checkbox" type="checkbox" id="cinema_{{ $cinema->id }}"
                                    name="cinemas[]" value="{{ $cinema->id }}" checked>
                                ...
                                <div class="timing-row mb-2">
                                    <div class="row">
                                        <div class="col-md-5">
                                            ...
                                            <select class="form-select cinema-timing" name="cinema_timings[{{ $cinema->id }}][]" required>
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <input type="date" class="form-control show-date" name="show_dates[{{ $cinema->id }}][]" required>
                                        </div>
                                        <div class="col-md-2 d-flex align-items-end">
                                            <button type="button" class="btn btn-success add-timing" data-cinema-id="{{ $cinema->id }}">+</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.cinema-checkbox').on('change', function() {
                ...
                cinemaRow.find('.timing-container input, .timing-container select').prop('disabled', !isChecked);
            ...
            $(document).on('click', '.add-timing', function() {
                ...
                const cinemaTimings = {!! json_encode($cinemas->keyBy('id')->map(function($cinema) {
            ...
            handleAjaxFormSubmit('#assignMoviesForm', '{{ route('assign.movies.store') }}', '{{ route('assign.movies.index') }}');
        });
    </script>
@endpush